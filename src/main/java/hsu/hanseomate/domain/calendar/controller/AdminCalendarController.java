...
@Tag(
        name = "관리자 학생회 일정 관리",
        description = "ADMIN 권한으로 학생회 일정을 조회, 등록, 수정, 삭제합니다."
)
@Validated
@RestController
...
public class AdminCalendarController {
    ...
    @Operation(summary = "관리자용 학생회 일정 전체 조회")
    @ApiResponses({
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "학생회 일정 없음",
                    content = @Content(schema = @Schema(implementation = ApiErrorResponse.class))
            )
    })
    @PutMapping("/{calendarId}")
    public CalendarEventResponse updateEvent(
            @Parameter(description = "학생회 일정 ID", required = true)
            @Positive(message = "일정 ID는 1 이상이어야 합니다.")
            @PathVariable Long calendarId,
            @Valid @RequestBody CalendarEventRequest request
    ) {
        return calendarEventService.updateEvent(calendarId, request);
    }

    @Operation(summary = "학생회 일정 삭제")
    @ApiResponses({
