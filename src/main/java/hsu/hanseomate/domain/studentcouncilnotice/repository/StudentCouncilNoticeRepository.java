...
public interface StudentCouncilNoticeRepository extends JpaRepository<StudentCouncilNotice, Long> {
    ...
    @Query("SELECT notice FROM StudentCouncilNotice notice WHERE notice.id = :noticeId")
    Optional<StudentCouncilNotice> findByIdForUpdate(@Param("noticeId") Long noticeId);
