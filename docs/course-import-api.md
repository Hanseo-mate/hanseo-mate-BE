...
- 같은 학기 전공 재수입은 같은 학기 전공의 활성 상태만 갱신하며, 같은 학기 교양과 다른
  학기의 데이터는 유지한다.
- 전공 시간표가 `STORED`로 저장되면 강의실 건물을 기존 건물·별칭 데이터로 정규화하고,
  `학과 + 건물`별 서로 다른 강좌 수를 별도로 집계한다. 같은 강좌가 같은 건물에서 여러
  시간대로 진행되어도 그 건물에는 한 번만 센다.
- 한 건물에서 서로 다른 강좌가 3개 이상인 학과만 교내시설 상세의 `departments`에
  저장한다. 새 전공 시간표가 성공적으로 저장될 때 기존의 모든 건물별 학과 목록을 새
  집계 결과로 교체한다.
- `DUPLICATE`, `REVIEW_REQUIRED`, 교양 시간표 업로드는 건물별 학과 목록을 변경하지
  않는다. 건물 또는 캠퍼스를 기존 건물·별칭 데이터로 확정할 수 없는 강의실은 집계에서
  제외한다.
