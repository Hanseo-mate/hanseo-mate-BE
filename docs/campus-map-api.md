...
- 교내시설은 등록·수정 시 `lectureBuildingDetails.location`의 앞뒤 공백을 제거한 값을 `address`에도 자동 저장합니다. 요청의 `address` 값보다 `location`을 우선하며, 목록·상세 응답 모두 저장된 주소를 반환합니다. 기존 데이터는 다시 저장할 때 반영됩니다.
