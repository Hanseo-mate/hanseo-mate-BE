...
일반 사용자용 `GET /api/clubs/{clubId}`와 동일한 상세 정보를 반환한다.
...
`recruitmentContent`가 새로 작성되거나 기존 값과 다르게 수정되면 해당 시점에
동아리를 좋아요한 로그인 사용자에게만 모집공고 알림을 생성한다.
모집공고가 동일하거나 다른 동아리 정보만 변경된 경우에는 알림을 생성하지 않는다.
