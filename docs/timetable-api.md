...
`Course`는 학년도·학기·교육과정 유형·과목코드·분반 조합별로 과목명·교수·수업 시간·
강의실·분류 등 강좌 정보를 저장한다. `CourseOffering`은 수입 출처와 활성 상태를
나타내므로 시간표는 학기 유효성을 확인할 수 있는 Offering UUID를 참조한다.

같은 과목코드라도 분반이 다르면 서로 다른 Course와 Offering UUID를 사용한다. 따라서
과목명이 같은 분반들이 검색 결과와 시간표에 각각 표시되고, 교수·수업 시간·강의실과
충돌 검사도 분반별 데이터로 처리한다. 같은 과목코드·분반 조합도 학기가 다르면 Course와
Offering UUID를 모두 분리한다. 같은 학기·같은 분반을 재수입할 때는 기존 Offering UUID를
유지하면서 상세정보를 최신 파일 값으로 교체하므로 이미 시간표에 추가한 항목도 유지된다.
