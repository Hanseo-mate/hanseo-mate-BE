...
`Course`는 과목코드와 분반 조합별로 최초 수입한 과목명·교수·수업 시간·강의실·분류 등
강좌 정보를 저장한다. `CourseOffering`은 이 강좌가 개설된 학기와 수입 출처를
...
충돌 검사도 분반별 데이터로 처리한다. 같은 과목코드·분반 조합이 여러 학기에 개설되면
Course는 공유하지만 학기별 Offering UUID는 서로 다르다. 같은 학기·같은 분반을 재수입할
때는 기존 Offering UUID를 유지하므로 이미 시간표에 추가한 항목도 유지된다.
