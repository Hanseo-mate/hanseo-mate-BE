...
기존 일정의 세 필드를 모두 전달하는 전체 수정 방식입니다.
...
| HTTP 상태 | 발생 상황 |
|---:|---|
| `400 Bad Request` | 필수값 누락, 공백 제목, 날짜 형식 오류, 종료일이 시작일보다 빠름, 잘못된 일정 ID |
...
기존 운영 DB에 캘린더 테이블이 이미 있다면 애플리케이션 배포 전에 더 이상 사용하지 않는 `content` 컬럼을 제거해야 합니다.

```sql
ALTER TABLE student_council_calendar_events
    DROP COLUMN content;
```

캘린더 테이블이 아직 없는 새 DB에는 다음 생성문을 적용합니다. 기존 운영 DB에는 `docs/database-schema-mysql.sql` 전체를 실행하지 않습니다.
